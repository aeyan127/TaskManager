            ...
            <tbody>
                {tableData.map((task) => (
                    <tr key={task._id} className="border-t border-gray-200">
                        <td className="my-3 mx-4 py-4 text-gray-700 text-xs line-clamp-1 overflow-hidden">{task.title}</td>
                        ...
                        <td className="py-4 px-4 text-gray-700 text-xs text-nowrap hidden md:table-cell">{task.createdAt?moment(task.createdAt).format('Do MMM YYYY'):'N/A'}</td>
                    </tr>
                ))}
            </tbody>
        </table>
